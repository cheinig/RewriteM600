...
class Rewritem600Plugin(octoprint.plugin.AssetPlugin, octoprint.plugin.TemplatePlugin, octoprint.plugin.SettingsPlugin):
    def rewrite_m600(self, comm_instance, phase, cmd, cmd_type, gcode, *args, **kwargs):
        if gcode and gcode == "M600":
            self._logger.info("catching M600 code to pause print")
    ...
    def before_resume(self, comm_instance, script_type, script_name, *args, **kwargs):
        if script_name == "beforePrintResumed":
            self._logger.info("M600 before_resume handler handles resume")
            if (comm_instance.pause_position.x):
                ...
                cmd = ["M83",
                       ...
                       "G1 X"+str(comm_instance.pause_position.x)+" Y" +str(comm_instance.pause_position.y)+" F4500",
                       "G1 Z"+str(comm_instance.pause_position.z)+" F4500",
                       "M83",
                       "G1 E1 F4500"]
                ...
                self._logger.info(
                    "no paused position available. skipping position reset")
        return
...
def __plugin_load__():
    ...
    __plugin_hooks__ = {
        ...
        "octoprint.comm.protocol.scripts": __plugin_implementation__.before_resume,
    }
